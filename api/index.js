export default async function handler(req, res) {
    // 1. Izin CORS
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', '*');

    if (req.method === 'OPTIONS') {
        return res.status(200).end();
    }

    try {
        // 2. Tembak API 55five dengan PENYAMARAN SUPER LENGKAP
        const response = await fetch("https://api.55fiveapi.com/api/webapi/GetNoaverageEmerdList", {
            method: "POST",
            headers: {
                "Content-Type": "application/json;charset=UTF-8",
                "Accept": "application/json, text/plain, */*",
                // TOKEN BEARER ASLI MILIKMU:
                "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..kYQ3AVeR2jbEovWMH4L73ukXfJBZeXXaqRj1CtRoWHw",
                
                // --- PENYAMARAN PENTING YANG SEBELUMNYA KURANG ---
                "Ar-Origin": "https://www.lopmiva.com",
                "Referer": "https://www.lopmiva.com/#/home/AllLotteryGames/WinGo?id=1",
                "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Mobile Safari/537.36"
            },
            body: JSON.stringify({
                "pageSize": 10,
                "pageNo": 1,
                "typeId": 1,
                "language": 1,
                // Pastikan Random, Signature & Timestamp ini sesuai dengan cURL mu
                "random": "8e9c62a11dca40e4a0717ee4bdc5eb77",
                "signature": "F3491A7019E3FA020176E3BF6890D0CE",
                "timestamp": 1777750742
            })
        });

        // 3. Baca respon sebagai teks dulu (sistem anti-crash)
        const textResponse = await response.text();

        try {
            // Coba terjemahkan teks menjadi JSON
            const result = JSON.parse(textResponse);
            
            if (result && result.data && result.data.list && result.data.list.length > 0) {
                const latest = result.data.list[0];
                return res.status(200).json({
                    periode: latest.issueNumber,
                    hasil: latest.number
                });
            } else {
                return res.status(500).json({ error: "Format JSON 55five berubah / Data kosong", raw: result });
            }

        } catch (err) {
            // Jika masuk ke sini, artinya Token Expired ATAU masih diblokir Cloudflare
            return res.status(500).json({ 
                error: "DIBLOKIR: Token Bearer mungkin sudah mati (Expired) atau WAF aktif.", 
                htmlTangkapan: textResponse.substring(0, 150) + "..."
            });
        }

    } catch (error) {
        return res.status(500).json({ error: error.message });
    }
}
